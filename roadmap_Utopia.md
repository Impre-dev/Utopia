...
|---|---|---|
| 0.1.0 | Fork initial : copie renommée de Nebula-Fork (`NebulaFork`→`Utopia`, `nebula-*`→`utopia-*` : attributs, vars CSS, ids overlays, log prefix). Cohabite avec Nebula-Fork (namespaces disjoints) pour A/B. | *(à pousser)* |

## 🎯 File priorisée

### 1. Premier install & validation de la copie ⭐
**Demande** : « créer un nouveau thème sans détruire l'ancien, pour modifier sans crainte »
**Implémentation** : git init + push GitHub public → Sine UI Add `Impre-dev/Utopia` → restart → vérifier `[Utopia] Initializing transparency engine...` en console + rendu identique au fork (glass, blur, tabs, veil).
**Critère** : comportement visuel identique au fork en cohabitation.

### 2. Scoping au combo de prefs cible
...
### 3. Bordure du conteneur web (bloc A de test.css) en option
...
### 5. Bascule finale : désactiver Nebula-Fork
...
### 6. Enquête : règle morte `[utopia-zen-gradient-contrast-zero]`
