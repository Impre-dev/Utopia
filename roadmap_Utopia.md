...
6. **`selected`/`visuallyselected` sont des attributs booléens VIDES sur les tabs Zen** (`selected=""`, pas `selected="true"` — prouvé en console 06/09 via getComputedStyle + dump attributs). Conséquence : `[selected='true']` ne matche jamais → les états actifs ne s'appliquent pas silencieusement. Contournement : matcher la présence `:is([selected], [visuallyselected])` — matche aussi bien `=""` que `"true"`, robuste cross-version. Leçon générale : quand une règle d'état ne s'applique pas, dumper `[...tab.attributes]` en console avant d'incriminer la spécificité.
